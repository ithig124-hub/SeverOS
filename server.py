...
def api_themes():
    """Serve available themes."""
    theme_file = os.path.join(DATA_DIR, 'themes.json')
...
def api_storage_health():
    """Return disk health for all mounted filesystems."""
...
def api_storage_drives():
    """Return list of external/removable drives."""
...
def api_storage_mount():
    ...
    ok, msg = mount_drive(device, data.get('mountpoint'))
...
def api_storage_unmount():
    """Unmount a drive by mountpoint."""
    data = request.get_json() or {}
    mountpoint = data.get('mountpoint')
    if not mountpoint:
        return jsonify({'success': False, 'message': 'mountpoint is required'}), 400
    from core.storage_manager import unmount_drive
    ok, msg = unmount_drive(mountpoint)
    return jsonify({'success': ok, 'message': msg}), (200 if ok else 500)
